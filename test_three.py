...
@pytest.mark.parametrize("grade_list, mean, sd", [
    ([90, 73, 88, 64, 99], 82.8, 12.576),
])

def test_calculate_stat(grade_list, mean, sd):
    mean_calc = calculate_stat(grade_list)
    sd_calc = calculate_stat(grade_list)
    assert mean_calc == mean
    assert sd_calc == sd